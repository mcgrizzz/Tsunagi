...
| 1 | `answerCards`, `addNotes`, `insertReviews` | Match order of mutation, partial success, rollback boundaries, errors and undo state. `answerCards` currently prevalidates malformed entries; `insertReviews` is transactional where upstream may leave preceding writes. The existing `addNotes` rollback test covers one failure sequence only. |
