...
def rpc(action, settings, origin=None, key=None, ask=None):
    req = AnkiConnectRequest(action=action, key=key)
    return handle_ankiconnect_rpc(req, origin=origin, settings=settings, ask_permission=ask)
...
class TestEnvelopeOverHttp:
    ...
    def client(self):
        ...
        @app.post("/")
        def endpoint(body: AnkiConnectRequest, request: Request):
